...
- **Fix: invoices huérfanos cuando pierden la carrera contra su contrato (G-99), desplegado 2026-09-21** — `reconcile-inventory-batch` **v22** en producción (v43 en staging), `main` `b2d775a` (PR #10). En un batch de inventario con vendor nuevo, un invoice que termina antes que su contrato vuelve a quedar vinculado a ese contrato (`contract_id`, `vendor_match_status = pending`) al cerrar el batch, igual que los que llegan después. Desde R-044 (2026-07-17) quedaban `unmatched` de forma permanente, porque la reparación consultaba columnas que `contracts` no tiene y fallaba en silencio. **No repara los casos viejos**: ese backfill es G-100, sin autorizar.
...
- **Billing/IRW fixes (R-044, R-039, R-027), deployed 2026-07-17** — R-044: same-vendor contract disambiguation in orphan-invoice matching now breaks name-similarity ties using billing-date overlap (fixed a real Third Bridge mismatch). ⚠️ **Corrección 2026-09-21:** el desempate de R-044 **nunca corrió en runtime** hasta el 2026-09-21. El mismo cambio rompió el select de `linkOrphanInvoicesToDraftContracts` (pedía `contracts.start_date/end_date`, que no existen), así que la función retornaba antes de llegar al desempate. El caso Third Bridge se validó sólo con tests en memoria. Restaurado por G-99. R-039: `get_invoice_reconciliation`/`create_invoice_distributions`'s account-based service inclusion now requires `annual_value > 0`, excluding unpriced/unallocated placeholder services (fixed on Mimecast, regression-checked against the legitimate Third Bridge cross-contract case); also fixed "+Add Service" showing with nothing addable. R-027: a new trigger syncs `invoice_adjustments` into `inventory_period_snapshots` when the adjustment targets an already-closed period — mirrors what Senthio does manually via `Adj Inv`/`Adj Year`/`Adj Month`; deliberately did **not** add automatic date-to-period inference elsewhere (tried it, reverted — that's a human judgment call there too).
...
### S4 Market Data - Demo — dataset 100% ficticio (staging **y produccion**, 2026-08-25)
...
- **Cargado también en producción el 2026-08-25**, con el mismo `org_id`. Los guards ya no son "staging only": leen `demo_seed.target`, con **default `staging`**, así que la versión que está en git nunca apunta a prod y hay que pedirlo explícito en la invocación (`SET demo_seed.target='prod';`). Verificado contra la base de prod real que **sin el `SET` no pasa nada**: el default aborta ahí.
- **Los dos entornos son idénticos al centavo**: 82 facturas, subtotal $1.304.265,00, impuesto $52.203,20, total $1.356.468,20. En prod entran **Santi y Ed** como admin (en staging solo Ed, que es el único que la necesita).
...
- **Accesos**: en **producción**, Santi (`c0dfaa83…`) y Ed (`ea366b61…`), los dos admin — verificado en vivo. En **staging**, solo Ed: Santi no tiene cuenta ahí y no la necesita, porque la demo sale de producción. **No se crea ningún usuario**: el paso 1 solo inserta la membresía resolviendo el `user_id` por email, así que le da acceso a quien exista en ese entorno y no hay nada que ajustar por entorno.
...
- **Verificado**: las 84 facturas originales con `net_variance` 0.0000 por los dos caminos de `get_invoice_reconciliation` (snapshots congelados y suscripciones vivas); las signed URLs resuelven sin autenticación devolviendo `application/pdf`. **Validación visual de Edgar: staging el 2026-08-21, producción el 2026-08-25.**
- ⚠️ **El dataset tiene fecha de vencimiento.** Está anclado a ene–ago 2026, con agosto como período abierto, y el gráfico de renovaciones del Dashboard deriva su ventana de 12 meses de *la fecha de hoy* (`buildRenewalMonths`, `Dashboard.tsx`). Cargado el 2026-08-25 cuadra. Si se usa meses después, el período "abierto" queda viejo y la ventana del gráfico se corre — hay que re-fechar el dataset (correr el revert y recargar con las fechas movidas). Aplica a los dos entornos.
- ⚠️ **Producción está viva mientras uno opera ahí.** Durante la ventana de carga, HIG sumó 7 facturas reales de otro origen. Un conteo tomado antes y comparado después **no prueba aislamiento**; lo que lo prueba es el scoping por `org_id`, que es lo que se verificó (cero vendors ficticios fuera de la org demo en toda la base).
- **96 PDFs en los dos entornos**, verificados con las signed URLs traídas sin autenticación (HTTP 200, `application/pdf`) y con los conteos de la base cuadrando contra los objetos del bucket. Cero URLs apuntando al proyecto equivocado.
...
- **Nombres medidos también contra el padrón de producción** (357 vendors) el 2026-08-25. Máximo 0.421 — *Ironwood Reference Data* ~ *ICE Data Pricing & Reference Data, LLC*, sin cambios respecto de staging.
  Ese chequeo hizo renombrar uno: *Ashford Security Ltd.* puntuaba **0.455** contra *Abira Security Corporation*, un vendor que existe en prod y no en staging. Bajo el umbral de 0.5, pero con un margen de 0.045 que no sobrevive a que entre otro vendor con "Security" en la razón social. El problema era la palabra, no el nombre inventado. Pasó a **Ravensgate Endpoint Ltd.**, que puntúa **0.148** en los dos padrones y 0.023 contra los otros nueve ficticios — margen de 0.352. El código de contrato `ASHFD-01` se dejó igual a propósito: tiene índice único, cuelgan referencias, y el código no tiene por qué seguir al nombre del vendor.
  El nombre estaba desnormalizado en **seis tablas** (`vendors`, `contracts.contract_vendor`, `invoices.vendor_name` + `matched_vendor_name`, `inventory_period_snapshots`, `invoice_distributions`, `inventory_upload_items`) y dentro de 8 PDFs. Reports lee de los snapshots, no de `vendors`, así que tocar solo la tabla de vendors habría dejado dos nombres distintos en pantalla. Ver `scripts/demo-seed-fictional/71_rename_ashford.sql`; el nombre nuevo ya está en `20_`/`30_`, así que una carga desde cero nace correcta.
...
## Recent changes (2026-09-21 session — G-99: invoices huérfanos por la carrera contrato/invoice, desplegado en staging y producción)

**El bug.** Visto en producción: un invoice subido en el mismo batch que su contrato quedaba sin match. Se reprodujo en staging (batch `c6307749`, org *S4 Market Data - Demo*). Los workers de un batch corren en paralelo. Un invoice cuyo vendor todavía no existe busca un contrato Draft del batch ya terminado; si llega antes (el de FactSet llegó 6,26 s antes), nace `unmatched` y sin `contract_id`. Para eso existe una reparación al cerrar el batch, `linkOrphanInvoicesToDraftContracts` en `reconcile-inventory-batch`, pero desde R-044 (`5b66ca4`, 2026-07-17) consultaba `contracts.start_date/end_date`, columnas que no existen (son de `services`). PostgREST devolvía `400 / 42703`, el código no leía el error, y la reparación se salteaba sin escribir ni loguear nada. Confirmado en runtime con los logs del gateway (`GET 400` durante reconcile, sin el log `orphan invoice-to-contract linking`) y con el mismo `42703` medido en producción.

**El fix (PR #10, `b2d775a`).** Dos cambios, sólo en `reconcile-inventory-batch/index.ts`: el select usa alias (`start_date:current_period_start, end_date:current_period_end`), con lo que `planOrphanInvoiceLinks` recibe la misma forma sin tocarse; y las dos consultas leen y loguean su `error`, así que un fallo así ya no pasa en silencio. Antes de escribirlo se revisó el blast radius: un solo llamador; el upsert parcial sobre `invoices` es seguro, porque no hay columnas NOT NULL sin default, el único trigger es AFTER y el default de `status` pasa el CHECK; y el estado resultante es el mismo que ya produce el worker.
...
**Proceso — primer cambio de backend que recorrió los Steps 5-7 desde la revisión del 08-21.** Anduvo lo que tenía que andar: commit antes de deployar, diff del bundle vivo antes y después en los dos entornos, CI verde antes de mergear, producción en la misma sesión. Roces que conviene conocer:
